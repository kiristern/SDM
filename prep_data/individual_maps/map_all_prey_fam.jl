...
# Import csv file
df = CSV.read("data/prey_family/prey_fam_onedf.csv", header=true)

#remove rows with missing values
df_drop = dropmissing(df, [:decimalLatitude, :decimalLongitude, :year])

#new df of just genus, occurence, year
df_new = select(df_drop, [:family, :decimalLatitude, :decimalLongitude, :year])
...
df_new = dropmissing(df_new, [:species])
